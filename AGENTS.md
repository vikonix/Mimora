...
- [`echoloop/tts.py`](echoloop/tts.py) — `TTSManager`: Kokoro TTS. `synthesize()` returns the waveform; `play_array(waveform, sample_rate)` plays any waveform. Also exports `reset_portaudio()` (shared PortAudio reset used by recording and playback), `uses_winsound()` (single source of truth for which playback path is taken), and `loudness_envelope(waveform, sample_rate, fps)` (per-frame RMS → 0..1 track that drives the talking mouth — see [`face_widget.py`](echoloop/face_widget.py)).
