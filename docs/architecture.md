...
| `mimora/llama_server_fetch.py` | Standalone, stdlib-only downloader for a pinned llama.cpp release into `bin/llama/`: sha256 per asset, staged unpack, then `--version` and `--list-devices` probes that catch llama.cpp's silent fallback to CPU. Run by hand (`python -m mimora.llama_server_fetch`); nothing calls it automatically yet. |
