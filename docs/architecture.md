...
| `mimora/llama_server_fetch.py` | Standalone, stdlib-only downloader for a pinned llama.cpp release into `bin/llama/`: sha256 per asset, staged unpack, then `--version` and `--list-devices` probes that catch llama.cpp's silent fallback to CPU. The download is run by hand (`python -m mimora.llama_server_fetch`); the app only reads what is already installed. |
